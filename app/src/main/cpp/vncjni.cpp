...
Java_com_a2k_vncserver_VncJni_protoGetVersion(JNIEnv *env, jobject obj) {
    return env->NewStringUTF(VncServer::getInstance().getVersion().c_str());
}

JNIEXPORT void JNICALL
Java_com_a2k_vncserver_VncJni_bindNextGraphicBuffer(JNIEnv *env, jobject obj) {
    VncServer::getInstance().bindNextProducerBuffer();
}

JNIEXPORT void JNICALL
Java_com_a2k_vncserver_VncJni_frameAvailable(JNIEnv *env, jobject obj) {
    VncServer::getInstance().frameAvailable();
}

JNIEXPORT jint JNICALL
Java_com_a2k_vncserver_VncJni_startServer(JNIEnv *env, jobject obj,
                                          jint width, jint height,
                                          jint pixelFormat,
                                          jboolean fullFrameUpdate) {
    return VncServer::getInstance().startServer(width, height, pixelFormat,
                                                fullFrameUpdate);
}

JNIEXPORT jint JNICALL
Java_com_a2k_vncserver_VncJni_stopServer(JNIEnv *env, jobject obj) {
    return VncServer::getInstance().stopServer();
}

JNIEXPORT void JNICALL
Java_com_a2k_vncserver_VncJni_onRotation(JNIEnv *env, jobject obj,
                                         jint rotation) {
